...
-- PROCEDURE DA FOLHA DE PAGAMENTO
...
EXEC PROC_FOLHA 3,2026,'2026-03-05'
...
BEGIN  
    --APAGA REGISTRO PARA RECALCULAR
    ...
    --CURSOR PARA LER SALARIO BRUTO
    FOR SAL_BRUTO IN (
        SELECT A.COD_EMPRESA,A.MATRICULA,A.SALARIO AS SALARIO_BRUTO   
        FROM SALARIO A
        ...
        --CURSOR PARA CALCULAR CONTRIBUICAO DO INSS
        FOR C_INSS IN (
            SELECT A.PCT AS PCT_INSS 
            FROM PARAM_INSS A
            ...
            --REGRA DE TETO DE INSS QUANDO SALARIO MAIOR QUE 8475.56
            IF SAL_BRUTO.SALARIO_BRUTO>8475.56 THEN
                VAL_INSS:=1695.11;
                C_INSS.PCT_INSS:=0;	 
            ELSE
            ...
            --CURSOR PARA CALCULO DE IRRF	
            FOR C_IRRF IN (
                SELECT A.PCT AS PCT_IRPF, A.VAL_ISENT  
                FROM PARAM_IRRF A
                WHERE SAL_BRUTO.SALARIO_BRUTO BETWEEN A.VALOR_DE AND A.VALOR_ATE
                    AND P_MES_REF BETWEEN TO_CHAR(A.VIGENCIA_INI,'MM') AND TO_CHAR(A.VIGENCIA_FIM,'MM')
                    AND P_ANO_REF BETWEEN TO_CHAR(A.VIGENCIA_INI,'YYYY') AND TO_CHAR(A.VIGENCIA_FIM,'YYYY') )LOOP
...
SET SERVEROUTPUT ON
EXECUTE PROC_FOLHA (1,'02','2026','05/03/2026');
